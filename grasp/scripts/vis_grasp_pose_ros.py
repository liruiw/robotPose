...
def get_table_offset(mesh_idx, pose):
    points = np.loadtxt('models/{}/points.xyz'.format(classes[mesh_idx]))
    points = pose[:3, :3].dot(points.T) + pose[:3, 3].reshape([3, 1])
    z = points[2]
    min_idx = np.argmin(z)
    table_size = np.array([-0.9, -0.5, -0.8]) #from inspection is probably easier
    table_translation = points[:, min_idx] + table_size
...
def render_pose(curr_base_pose, poses):
    ...
    if joints is not None:
        ...
        for i in range(len(pose_scene)):
            tf = camera_extrinsics.dot(pose_scene[i])
            trans = tf[:3, 3]
            rot = mat2quat(tf[:3, :3])
...
def example_publish(curr_base_pose):
    global mat_poses, num, current_pose_grasp, object_pose, gripperPose
    while not rospy.is_shutdown():
        if current_pose_grasp is not None:
            ...
            for i, cls in enumerate(cls_indexes):
                ...
                object_pose.append(inv_cam_pose.dot(pose))
            ...
            msg = Pose() #publish table pose
            table = np.eye(4) #z up
            table[:3, 3] = get_table_offset(target, object_pose[target_idx]) 
            quat = mat2quat(table[:3, :3])
            msg.orientation.x = quat[1]
            msg.orientation.y = quat[2]
            msg.orientation.z = quat[3]
            msg.orientation.w = quat[0]
            msg.position.x = table[0, 3]
            msg.position.y = table[1, 3]
            msg.position.z = table[2, 3]
...
if __name__ == '__main__':
    ...
    robot_pub = rospy.Publisher('robot_pose', Pose, queue_size=1)
    table_pub = rospy.Publisher('table_pose', Pose, queue_size=1)
